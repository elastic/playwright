...
class Recorder {
  ...
  async installRecorder(recorderApp) {
  ...
  async _install(recorderApp, showRecorder) {
    if (showRecorder)
    ...
    await this._context.exposeBinding("__pw_recorderElementPicked", false, async ({ frame }, elementInfo) => {
      ...
      const selector = (0, import_recorderUtils.buildFullSelector)(selectorChain, elementInfo.selector);
