...
class Tracing {
  ...
  async start(options) {
    ...
    if (!state) {
      ...
      this._writeChain = (0, _utils.mkdirIfNeeded)(traceFile).then(() => _fs.default.promises.writeFile(networkFile, ''));
    ...
    await this._appendTraceOperation(async () => {
      ...
      if (state) {
        state.lastReset++;
        state.traceFile = _path.default.join(this._tracesDir, `${state.traceName}-${state.lastReset}.trace`);
  ...
  onResourceSnapshot(snapshot) {
    ...
    this._appendTraceOperation(async () => {
      ...
      await _fs.default.promises.appendFile(this._recording.networkFile, JSON.stringify(event) + '\n');
    });
  }
