...
class ElementHandle extends js.JSHandle {
  ...
  async inputValue() {
    return throwFatalDOMError(await this.evaluateInUtility(([injeced, node]) => {
      if (node.nodeType !== Node.ELEMENT_NODE || node.nodeName !== 'INPUT' && node.nodeName !== 'TEXTAREA') return 'error:notinputvalue';
      const element = node;
      return {
        value: element.value
      };
...
function throwFatalDOMError(result) {
  ...
  if (result === 'error:notinputvalue') throw new Error('Node is not an HTMLInputElement or HTMLTextAreaElement');
...
function inputValueTask(selector) {
  return injectedScript => injectedScript.evaluateHandle((injected, {
    ...
    return injected.pollRaf((progress, continuePolling) => {
      ...
      if (!element) return continuePolling;
      progress.log(`  selector resolved to ${injected.previewNode(element)}`);
      if (element.nodeName !== 'INPUT' && element.nodeName !== 'TEXTAREA') return 'error:notinputvalue';
