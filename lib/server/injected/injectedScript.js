...
class InjectedScript {
  ...
  querySelector(selector, root, strict) {
    if (!root['querySelector']) throw this.createStacklessError('Node is not queryable.');
  ...
  checkElementState(node, state) {
    ...
    if (state === 'checked') {
      if (['checkbox', 'radio'].includes(element.getAttribute('role') || '')) return element.getAttribute('aria-checked') === 'true';
  ...
  strictModeViolationError(selector, matches) {
    const infos = matches.slice(0, 10).map(m => ({
      ...
      selector: (0, _selectorGenerator.generateSelector)(this, m).selector
    }));
