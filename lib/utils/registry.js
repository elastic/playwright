...
const os = _interopRequireWildcard(require("os"));
...
const _properLockfile = _interopRequireDefault(require("proper-lockfile"));

const _ubuntuVersion = require("./ubuntuVersion");

const _utils = require("./utils");

const _dependencies = require("./dependencies");

const _browserFetcher = require("./browserFetcher");
...
class Registry {
  ...
  _addRequirementsAndDedupe(executables) {
    const set = new Set();
    if (!executables) {
      executables = this._executables.filter(
        (executable) => executable.installType === "download-by-default"
      );
    }
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    const { code } = await (0, _utils.spawnAsync)(
      shell,
      [_path.default.join(BIN_PATH, scriptName), ...scriptArgs],
      {
        cwd: BIN_PATH,
        stdio: "inherit",
      }
